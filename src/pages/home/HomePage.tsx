...
export default function HomePage() {
  ...
  const renderedPackages = featuredPackages.map((p)=>{
    return <div key={p.name} className="flex flex-col justify-between gap-3 border rounded shadow p-4">
      <div className="flex flex-col gap1 border-bottom border-gray-400">
      ...
      <Link to={`/packages/${p.name}`} className="border rounded border-grey-900 text-center">
        View
      </Link>
    </div>
  })
  ...
  <div className="container py-12 space-y-8">
    <div className="space-y-6 text-center">
      ...
      <div className="max-auto grid grid-cols-4 max-w-[900px] items-stretch gap-4">
        {renderedPackages}
      </div>
  </div>
  )
